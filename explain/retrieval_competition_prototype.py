...
def explain_results(results: Sequence[PoolResult]) -> List[str]:
    if not results:
        return ["결과가 없습니다."]
    ...
    comments.append(
        (
            f"baseline에서는 target이 rank {baseline.target_rank}이고, "
            f"pool-softmax support는 {baseline.target_support_prob:.4f}입니다."
        )
    )
    ...
    biggest_drop = None
    for r in results[1:]:
        drop = baseline.target_support_prob - r.target_support_prob
        if biggest_drop is None or drop > biggest_drop[0]:
            biggest_drop = (drop, r)

    if biggest_drop is not None:
        drop_value, r = biggest_drop
        if drop_value > 0.20:
            comments.append(
                f"가장 큰 흔들림은 {r.level_name}에서 나타났고, support가 {drop_value:.4f}만큼 하락했습니다."
            )
        else:
            comments.append(
                f"가장 큰 흔들림도 {r.level_name}에서 {drop_value:.4f} 수준이라, 경쟁에 비교적 강한 편입니다."
            )
    ...
    if reversal_levels:
        ...
        comments.append(
            f"처음 top-1이 깨지는 지점은 {first.level_name}이며, 이때 가장 강한 rival은 {rival}입니다."
        )
    else:
...
def print_result_table(results: Sequence[PoolResult]) -> None:
    ...
    header = (
        ...
        f"{'rank':>4s} | {'top1':>4s} | strongest_rival"
    )
    print(header)
    print("-" * len(header))
    for r in results:
        ...
        print(
            ...
            f"{r.target_rank:4d} | {str(r.is_top1):>4s} | {rival}"
        )
    print()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="단일축 retrieval competition explanation prototype")
