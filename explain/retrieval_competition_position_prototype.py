...
# -----------------------------------------------------------------------------
# reader proxy 단계: positional accessibility
# -----------------------------------------------------------------------------
...
def compute_reader_proxy_logits(
    ...
    logits: List[float] = []
    for c, burial in zip(order, burial_scores):
        base_score = c.final_score if score_source == "final" else c.dense_score
        logits.append(float(retrieval_weight * base_score - position_penalty * burial))
    return logits



def evaluate_reader_position(
    competition_level: str,
...
def explain_position_results(
    position_results: Sequence[ReaderPositionResult],
    baseline_status: str,
) -> List[str]:
    if baseline_status == "target_mismatch_diagnostic":
        return [
    ...
    # competition level별로 front/middle/back 결과 묶기
    grouped: Dict[str, Dict[str, ReaderPositionResult]] = {}
    for r in position_results:
        grouped.setdefault(r.competition_level, {})[r.position_level] = r

    comments: List[str] = []

    # baseline 위치 민감도: L0에서 front vs middle 비교
    baseline_group = grouped.get("L0_weak_competition", {})
    front = baseline_group.get("front")
    middle = baseline_group.get("middle")
    back = baseline_group.get("back")
    if front and middle:
        drop = front.target_reader_support_prob - middle.target_reader_support_prob
        comments.append(
            f"reader proxy 기준 baseline(L0)에서 target을 front에서 middle로 옮기면 support가 {drop:.4f} 감소합니다. 즉 기본적으로 middle burial penalty가 존재합니다."
        )
    elif front:
        comments.append("reader proxy baseline 결과가 일부만 있어 front/middle position drop을 계산하지 못했습니다.")

    # 최초 position reversal 찾기
    first_pos_reversal: Optional[ReaderPositionResult] = None
    ...
    for comp in ordered_comp_levels:
        sub = grouped.get(comp, {})
        for pos in ["front", "middle", "back"]:
            r = sub.get(pos)
            if r is not None and not r.is_top1:
                first_pos_reversal = r
                break
        if first_pos_reversal is not None:
            break

    if first_pos_reversal is not None:
        rival = first_pos_reversal.strongest_rival_chunk_id or "unknown_rival"
        comments.append(
            f"reader proxy top-1이 처음 깨지는 지점은 competition={first_pos_reversal.competition_level}, position={first_pos_reversal.position_level}이며, strongest rival은 {rival}, margin은 {first_pos_reversal.margin_vs_rival:.4f}입니다."
        )
    else:
        comments.append("모든 competition × position 조합에서 reader proxy top-1을 유지했습니다.")

    hardest = min(
        position_results,
        key=lambda r: (r.margin_vs_rival if r.margin_vs_rival is not None else 0.0),
    )
    if hardest.margin_vs_rival is not None and hardest.margin_vs_rival < 0:
        comments.append(
            f"가장 불리한 reader 조건은 competition={hardest.competition_level}, position={hardest.position_level}이고, 여기서는 target이 {abs(hardest.margin_vs_rival):.4f}만큼 뒤집힙니다."
        )
    else:
        comments.append("가장 불리한 reader 조건에서도 음수 margin이 나타나지 않아 position 변화에 비교적 안정적입니다.")
...
def print_position_table(results: Sequence[ReaderPositionResult]) -> None:
    print("=" * 100)
    print("[READER POSITION RESULT TABLE]")
    header = (
        f"{'competition':24s} | {'position':8s} | {'ctx':>3s} | {'reader_logit':>12s} | {'support_p':>10s} | "
        f"{'rank':>4s} | {'top1':>4s} | {'burial':>8s} | {'margin':>10s} | strongest_rival"
    )
    print(header)
    print("-" * len(header))
    for r in results:
        ...
        margin_str = "-" if r.margin_vs_rival is None else f"{r.margin_vs_rival:10.6f}"
        print(
            f"{r.competition_level:24s} | {r.position_level:8s} | {r.context_size:3d} | {r.target_reader_logit:12.6f} | "
            f"{r.target_reader_support_prob:10.6f} | {r.target_reader_rank:4d} | {str(r.is_top1):>4s} | "
            f"{r.target_burial_score:8.4f} | {margin_str:>10s} | {rival}"
        )
    print()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="2축 prototype: retrieval competition × positional accessibility (reader proxy)")
    ...
    parser.add_argument("--skip-position-if-mismatch", action="store_true", help="baseline mismatch이면 position stage 계산 생략")
...
def main() -> None:
    ...
    position_results: List[ReaderPositionResult] = []
    if not (args.skip_position_if_mismatch and baseline_status == "target_mismatch_diagnostic"):
        for level_name, pool in level_pools:
            for position_level in ["front", "middle", "back"]:
                position_results.append(
                    evaluate_reader_position(
    ...
    retrieval_explanations = explain_retrieval_results(
        results=retrieval_results,
        baseline_status=baseline_status,
    )
    position_explanations = explain_position_results(
    ...
    print()
    if position_results:
        print_position_table(position_results)
    ...
    if args.write_json is not None:
        ...
        payload = {
            ...
            "reader_proxy": {
                ...
                "position_penalty": args.reader_position_penalty,
                "score_source": args.reader_score_source,
                "note": "reader-stage actual LLM score가 아니라 retrieval score와 burial penalty를 결합한 proxy입니다.",
            },
            "retrieval_results": [
                {
